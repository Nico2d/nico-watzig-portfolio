...
const ProjectCard = ({ thumbnail, title, description, stack }) => {
    return (
        <article className="bg-surface flex flex-col rounded-lg overflow-hidden h-min">
            {thumbnail && (
                <div className="relative aspect-[4/3] w-full h-full">
                    <Image className="object-cover" src={thumbnail} alt={"image"} fill />
                </div>
            )}
            ...
            <div className="flex-[2] px-5 py-6 text-center flex flex-col gap-10">
                <header className="flex-1 flex items-center justify-start flex-col gap-3">
                    ...
                    <p tabIndex={0} className="text-sm text-justify text-slate-300">
                        {description}
                    </p>
                </header>
