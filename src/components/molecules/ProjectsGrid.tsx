...
export const ProjectsGrid = ({ posts }: IProjectsGridProps) => {
    ...
    useEffect(() => {
        ...
        const updateColumnsData = () => {
            setColumnsData(splitListIntoColumns(posts, getRecommendedNumberOfColumns()));
        };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 ">
                {posts.map((post) => (
                    <ProjectItem
                        ...
                        thumbnail={getNotionPrivImage(post.Thumbnail, post.id, 500)}
                        slug={post.Slug}
                    />
                ))}
            </div>
...
function splitListIntoColumns(list: INotionProject[], numColumns: number): INotionProject[][] {
